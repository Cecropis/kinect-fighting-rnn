...
n_classes = 5  # 动作分类(右拳、左拳、右踢、左踢、防守)
...
def RNN(X, weights, biases):
    # 为了方便计算隐层输入
    X = tf.reshape(X, [-1, n_inputs])  # 将 shape=(256批, 200帧, 63) 的动作序列转换为 shape=(256*200, 63)
    X_in = tf.matmul(X, weights['in']) + biases['in']  # 计算出隐层输入
    X_in = tf.reshape(X_in, [-1, n_steps, n_hidden_units])  # 将隐层输入转换为 shape=(256批, 200帧, 128隐层神经元数)
    ...
    results = tf.matmul(states[1], weights['out']) + biases['out']  # shape=(256批, 5种动作)
    return results
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        batch_xs, batch_ys = kinect_data_loader.next_batch(batch_size)  # 读进来的数据 shape=(256 batch_size, 200帧, 21个点, 3 xyz)
        batch_xs = batch_xs.reshape([batch_size, n_steps, 63])  # 把上述 (256, 200, 21, 3) 转为 (256, 200, 63)
        sess.run([train_op], feed_dict={
            x: batch_xs,
            y: batch_ys
        })
        # 每20个epoch用一个新的batch测试精确度
        if step % 20 == 0:
